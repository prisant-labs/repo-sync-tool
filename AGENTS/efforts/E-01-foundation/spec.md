...
- [ ] AC8: The CI workflow pins the exact `git` version that the E-04 fixture harness depends on (so `git` porcelain/plumbing output stays byte-stable across runners and over time). E-04 specifies the stability requirement; E-01 owns the workflow YAML and this enforcement AC. Source: brief Section 6 (CI workstream); E-04 spec (pinned git for the fixture harness).
