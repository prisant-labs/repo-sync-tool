...
- **DataTable** (`src/components/ui/data-table.tsx`, N2). The shared table primitive, built to the table lab's settled defaults (`_local/gui/2026-08-28_iterations/README.md` and `_generators/gen_lab2.py`): sticky header, one frozen (sticky-left) column, horizontal scroll inside the table container, one cell-padding value everywhere, a data icon per column skipped on an empty cell, and two densities (52px "full", the lab's own default; 44px "compact", not lab-sourced - see the primitive's own doc comment). Header height is fixed at 32px in both densities. Presentational only: no sorting, no selection, no column show/hide (all deferred). The Repos screen is its first consumer; Activity is next (N3).
- **Repo table row.** Rebuilt on `DataTable` (N2). Columns, in order: Repository (frozen, the only flexible width; carries identity plus `IntelSignals`), Status (chip), Ahead, Behind, Groups (outlined pills with a colour dot), Checked, then an unlabeled actions cell (Check now icon button, a decorative chevron). Branch and Folder are NOT in this build: `RepoSummary` carries neither `activeBranch`/`branch` nor `localPath`, only `RepoDetail` does, and this change does not touch the backend to add them (see the new backlog row). The whole row is a button that opens the detail drawer; the actions cell stops propagation.
