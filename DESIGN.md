...
- **Graphite blue** `oklch(0.52 0.19 264)` light / `oklch(0.6 0.16 264)` dark; focus ring `--ring: oklch(0.62 0.15 264)`. Primary buttons, links, focus. Means "you can act here." **Never** used to convey repo status. The primary sidebar nav's active state moved OFF this accent as of N5 (sidebar restructure and toolbar consolidation, Components below); it still marks the active GROUP filter in the nested Groups section, which is a narrower, deliberate exception rather than a drift back.
...
- **Sidebar navigation** (`app-shell.tsx`, `groups-nav.tsx`, N5). Ratified order top to bottom: Dashboard, Activity, Repos, then the Groups section nested one level beneath Repos (indented, with a left guide rail - the materialization is provisional, no mockup specified an exact value), then Settings bottom-docked (`mt-auto`, separated by a hairline) rather than sitting in the primary list. The logo block (28px "R" square, "RepoSync" wordmark with "Sync" in accent, live version in 11px mono) is untouched. Active nav state moved OFF the accent tint (`bg-primary/10 text-primary`) onto the neutral 1B surface ramp: `bg-sidebar-accent` (the same well step `--muted` sits on) plus `font-semibold`, `text-foreground` - computed at 16.35:1 (light) / 14.48:1 (dark) against that surface, both far past the 4.5:1 floor (`_local/gui/2026-08-28_iterations/_generators/contrast.py`). Because `--muted` and `--sidebar-accent` are the same value in both themes, hover on an inactive item uses `bg-muted/60` rather than full opacity, so a hovered item and the active one stay perceptibly distinct (verified: active renders `oklch(0.935 0 0)` at full opacity plus weight 600; hover renders the same hue at 60% opacity and weight 500). The Groups section's own shipped behaviour (header, empty state, the "All repositories" clear row, per-group hover/focus-revealed rename+delete, the inline "Delete?" confirm, delete-clears-without-navigating, independent scroll) is unchanged by the move; its own active-row treatment still uses the accent tint, a narrower and deliberate exception named for veto in the N5 PR rather than a silent inconsistency.
...
- **Search + filter chips + group control** (`repos.tsx`, N5: one consolidated toolbar). A mono-ish search field; status filter chips carry the state color and a live count, the active chip in the accent, zero-count statuses hidden. The active group filter's own control (colour dot, name, member count, Clear) now rides in the SAME sticky toolbar rather than a separate banner beneath it - the fold is named as a decision, not a silent drop: the count reads an ellipsis, never a fabricated zero, while the bulk membership read is still in flight. The whole toolbar (search, chips, group control, Check all) is hidden together when the repo list is empty, which is a real behaviour delta from the old banner (it used to render regardless of list length); the sidebar's "All repositories" row is the filter's other, always-available clear path.
