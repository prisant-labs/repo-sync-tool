...
- Binary smoke gate (Windows): CI launches the built `reposync.exe` with a scratch data directory and fails unless it is still alive, with a non-empty log carrying its startup line AND the `app.startup_completed` readiness marker (non-empty catches a crash; the marker catches a startup hang, and, because the app withholds it when startup produced no usable window, a launch that ends with nothing on screen). A window failure never kills the app; the gate fails on the missing marker. Ruled a blocking CI gate (not a documented pre-release step) on 2026-09-04; see [BL-NI-88 (no gate ever launches the built binary)](docs/backlog.md). It runs as a step of the `build` job, so it blocks under the existing required check instead of adding a new one that branch protection would not require.
