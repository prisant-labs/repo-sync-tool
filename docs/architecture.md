...
- **`tauri-specta` generates the TypeScript bindings** from the Rust types at build time (guarded by `#[cfg(debug_assertions)]` so production builds do no file I/O). The frontend imports the generated typed `commands.*` and `events.*` instead of raw `invoke`/`listen`.
