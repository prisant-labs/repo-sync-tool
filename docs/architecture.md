...
- **A CI stale-bindings gate** regenerates `bindings.ts` and fails on any drift, so the committed bindings can never lie about the backend. The contract was frozen at E-06 (**18 commands and 8 events over roughly 25 payload types**, since extended additively by later efforts); commands were stubbed to return a typed `internal.unexpected` error until their backing efforts landed, so the surface was complete and stable while the behavior filled in. As of the v0.9.0 ship (2026-07-05) those backing efforts (E-07 through E-11, and the later E-16/E-17/E-18 additions) are all built.
