...
2. **The data-directory lock, before the Tauri builder exists.** `reposync_core::instance_lock::acquire` creates `reposync.lock` beside the database and takes an exclusive operating-system lock on it, held in `run`'s own scope for the life of the process. A launch that finds it already held logs `app.second_instance_db_locked` and RETURNS from `run` rather than calling `std::process::exit`, so every destructor runs, the log guard flushes, and the line survives. This is the backstop for the singleton's measured race, described below. A lock that cannot be established at all logs `app.instance_lock_unavailable` and the app starts anyway, with the singleton as its only guard.
