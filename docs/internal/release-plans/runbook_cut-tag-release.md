...
- [ ] `.github/workflows/release.yml` fires on the `v*` tag: builds Windows + macOS with the `dist` profile (full LTO) and creates a DRAFT GitHub Release with both platform artifacts attached, plus the `latest.json` updater manifest once E-18 (auto-update and distribution) has landed (see `plan_v0.9.0/E-18-auto-update/spec.md`). Verify `latest.json` is present on the draft's assets before moving to G4; its absence blocks the smoke test there.
...
- [ ] **Winget manifest submitted.** E-18 (auto-update and distribution) prepares the winget manifest during v0.9.0, but submission to `microsoft/winget-pkgs` waits until here: winget requires public artifact URLs that do not exist while the repo is private. Submit once the first public release's installer assets are live and stable.
- [ ] **Updater endpoint verified live.** Confirm the `tauri-plugin-updater` endpoint (the `latest.json` URL baked into the shipped installer) resolves publicly and unauthenticated, not merely from a collaborator's already-authenticated GitHub session. Private-repo release assets require auth to fetch; a public repo does not, but verify it directly rather than assuming the visibility flip alone fixes it.
