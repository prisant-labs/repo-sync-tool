...
- **Security hardening (2026-07-11, HTTP client):** `ReqwestTransport::new` now sets `.timeout(30s)`, `.connect_timeout(10s)`, and `.redirect(reqwest::redirect::Policy::none())`. reqwest has no default request timeout, so a half-open socket previously blocked the refresh task for the OS TCP timeout while holding the shared rate-budget lock (correctness CC-1), and following a cross-host redirect off the constant `api.github.com` host was an SSRF primitive (security R3). A refused redirect returns a 3xx that fails the existing `is_success()` check and degrades to `NetworkLost` (graceful, no panic). Residual (no response body-size cap) is tracked in BL-NI-49. clippy `-D warnings` clean. Addresses security review R3 + correctness CC-1.
