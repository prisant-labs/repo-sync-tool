...
5. **Configure the updater in `tauri.conf.json`.** Set `bundle.createUpdaterArtifacts: true`. Add `plugins.updater` with `pubkey` (from step 1), `endpoints: ["https://github.com/product-on-purpose/repo-sync-tool/releases/latest/download/latest.json"]` (the FINAL public URL, inert while private), and `windows.installMode: "passive"`. Confirm the NSIS artifact is the updater target (see step 8 / the NSIS-vs-MSI decision).
...
2. Confirm the live endpoint serves `latest.json` (`https://github.com/product-on-purpose/repo-sync-tool/releases/latest/download/latest.json` returns 200 with a valid manifest).
