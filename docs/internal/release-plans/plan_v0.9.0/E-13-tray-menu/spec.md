...
- **State:** CODE-COMPLETE for V1 (P3-C, 2026-07-05; dogfood-pending). `src-tauri/src/tray.rs` now builds the full native menu - **Show RepoSync**, **Check All Now**, **Pause all / Resume all** (label reflects the live global-pause state), **Open recent** (a submenu of the most-recently-active repos, each opening its folder via the hardened opener), **Settings** (shows + focuses the window and emits `navigate:requested` so the frontend routes to the settings view), **Quit** - plus left-click-show. **Close-to-tray** (AC3) is wired in `src-tauri/src/windows/mod.rs`: the close button hides the window and only Quit exits, and the main window is declared `visible: false` and shown explicitly on a normal launch (hidden on an autostart launch, consuming the E-15 start-minimized seam). **2026-07-30 update: close-to-tray is now a user setting, not hardcoded.** `Settings.closeMinimizesToTray` (migration `0007_close_minimizes_to_tray.sql`, default ON so every existing install keeps the prior behavior) decides whether the X hides to tray or exits; it is read live from a shared `AtomicBool` on every close, so toggling it in Settings takes effect with no restart. The decision is a pure, unit-tested `decide_close_action(tray_available, close_minimizes_to_tray)`, and **tray availability remains a hard gate the setting cannot override** - with no tray the close button always exits, preserving the finding-2 guarantee that a failed tray can never strand an invisible unquittable app. Tray Quit always exits either way. AC1/AC2/AC3/AC4/AC5 are all implemented; final launch verification is dogfood-only (see the smoke list in [../edge-wiring-plan.md](../edge-wiring-plan.md)). The frameless popover window stays cut to V1.1 (BL-V11-01).
...
- **Next:** dogfood the tray on a real Windows launch (the [edge-wiring-plan.md](../edge-wiring-plan.md) smoke checklist), then close AC1-AC5. The 2026-07-30 close-to-tray setting adds three checks to that dogfood: with the setting ON the X hides to tray (unchanged behavior), with it OFF the X quits cleanly, and tray Quit exits in both modes.
