...
- **Fail-closed hardening (2026-07-12, PS-1 / BL-NI-52):** the H-1 dirty re-check (`repo.rs:700`) read `unwrap_or(false)`, so a git2 inspect error at the pull instant resolved to "not dirty" and PROCEEDED to `pull --ff-only` - the one fail-OPEN site in an engine that fails closed everywhere else. Changed to `unwrap_or(true)` so an unreadable tree resolves to dirty and skips, matching every other unknown-state site (a status-read error already reads as dirty at the primary check). The never-destructive promise is unchanged (git's own `--ff-only` refusal was already the backstop); this closes the theoretical HEAD-advance-on-a-momentarily-unreadable-tree edge. Test `execute_action_h1_recheck_fails_closed_on_inspect_error`; clippy `-D warnings` clean.
- **What landed:**
  ...
  - The closed-`UpdateMode` invariant: non-V1 modes (`pull_standard`/`pull_rebase`) route to a `ModeNotAvailableInV1` skip (code `config.invalid_policy`), never executed as a V1 mode.
  - Every `SkipReason`/`RunOutcome` carries its stable `AppError` code (AC7).
  - Tests: 37 in-crate unit tests (the full grid + diverged row + dirty-precedence + non-V1 guard + AppError-code mappings + the full failure machine), plus a `tests/policy_fixture_matrix.rs` integration test that drives all 21 fixture cells through the REAL E-03 inspect engine and asserts the grid (feature `test-support`).
  - SECONDARY wiring: `pull --ff-only` added to the E-03 git CLI engine (`PullClass`/`PullOutcome` + `classify_pull`, mirroring the fetch-class pattern; 10 new tests incl. 2 real fast-forward/diverged e2e). `repo.rs::check_now` promoted to call `decide`. New shared `repo.rs::update_now` (the decide -> execute -> emit-data path E-08 reuses). `store.rs::repo_set_policy` persists the per-repo `update_mode` (rejects non-V1). Tauri handlers `repo_set_policy` + `repo_update_now` wired (with `update-started`/`update-completed` emission).
- **Next:** E-08 (scheduler) consumes `decide` + `classify_failure` + `update_now`; it persists `consecutive_failures`/`auto_paused`.
- **Blockers:** none.
- **Notes / follow-ups for jp:** (1) `UpdatePolicy.dirty_handling` and `branch_policy` are accepted by `repo_set_policy` but have no v1 schema columns, so they are not persisted (only `update_mode` is) - a named V1.1 extension point. (2) `decide` consumes a configured `UpdateMode` directly, not the full `UpdatePolicy`; the dirty-handling override would change that signature in V1.1. (3) bindings.ts regenerated with only docstring changes for the two wired handlers - the wire contract (types/signatures/events) is byte-identical.
...
- [x] AC1: `policy.rs` exposes a pure `(repo state, policy) -> action or skip-with-reason` function that performs no git, network, DB, or clock I/O. Source: brief Section 6 (Update-policy engine row: "Pure function ... No I/O, no git, no DB").
- [x] AC2: A decision table covers all 7 fixture states (clean, dirty, ahead, behind, detached HEAD, deleted-upstream, no-upstream) crossed with `check_only`, `fetch_only`, and `pull_ff_only`, each cell resolving to a defined action or typed skip-reason, matching the explicit grid in "In scope". The diverged (ahead-and-behind, e.g. the brief mockup's `up 2 down 5` repo) case is covered explicitly - tested within the behind row and resolving to `ff-not-possible` under `pull_ff_only` - not assumed. Source: brief Section 6 (Update-policy engine; fixture states) and Section 4.6.
- [x] AC3: Dirty handling never mutates a dirty working tree; `pull_ff_only` on a dirty repo yields a typed "skipped: dirty" reason. Source: brief Section 5, State semantics (the `### 4` subsection under `## 5`) (dirty: "Skipped per policy") and Section 6 (dirty handling).
- [x] AC4: Branch policy yields distinct typed skip-reasons for detached HEAD, no-upstream, and deleted-upstream, and pull is strictly fast-forward-only with an ff-not-possible reason when a behind state cannot fast-forward (including the diverged ahead-and-behind case). Source: brief Section 6 (branch policy, ff-not-possible) and Section 5, State semantics (the `### 4` subsection under `## 5`) (pull is "intentionally fast-forward-only and rebase was dropped").
- [x] AC5: The failure-handling state machine pauses on auth failure, retries on network failure, and signals auto-pause after 3 consecutive failures (reading the prior count from `repo_local_state.consecutive_failures`; E-08 persists the auto-pause as `auto_paused = 1`), resetting the consecutive counter on any success. Source: brief Section 6 (failure handling: "auth failure -> pause, network -> retry, 3 consecutive -> auto-pause").
- [x] AC6: The real invariant holds: any mode that is not one of the three V1 modes is never silently executed as a V1 mode - it returns a "mode not available in V1" skip-reason or is rejected at the boundary, enforced by the closed `UpdateMode` enum. Which future variants (if any) the enum declares is an agent default, since the brief names none (see Open questions); the invariant, not the variant list, is the contract.
- [x] AC7: Each skip-reason and failure outcome maps to a stable `AppError` code from E-05 (ff-not-possible, dirty, detached HEAD, deleted upstream, no upstream, auth failure, network lost). Source: brief Section 6 (`AppError` taxonomy row) and Section 4.6.
