...
>
> What has NOT changed: the updater is still **dark** (placeholder production signing key, a human-gated step), the installers are still **unsigned** on both platforms, and the winget submission has still not happened. Those were public-flip prerequisites and remain open. Current status lives in [the README](../../../../README.md#status-pre-release-and-honest-about-it) and the security posture in [docs/security-model.md](../../../security-model.md).
