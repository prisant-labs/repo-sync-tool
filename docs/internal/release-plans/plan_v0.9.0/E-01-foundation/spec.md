...
- **Security hardening (2026-07-16, WebView CSP / BL-NI-58):** `tauri.conf.json`'s `app.security.csp` was `null` (no Content-Security-Policy on the WebView). Replaced with a locked-down production `csp` (`default-src`/`script-src 'self'`; `connect-src ipc: http://ipc.localhost` only, no external hosts for script-interface loads (fetch/XHR/WebSocket); `object-src 'none'`; `base-uri`/`form-action 'self'`; `frame-ancestors 'none'`) plus a `devCsp` superset that additionally permits Vite HMR (`'unsafe-inline'`/`'unsafe-eval'`, `ws://localhost:1420`) - both are required because the config schema injects `csp` into dev too unless `devCsp` is set. `style-src` retains `'unsafe-inline'` (with a `style-src-attr` guard against Tauri's build-time style-nonce nullifying it) for the frontend's dynamic `style={}` props. Implements the CSP layer of the R5 threat (BL-NI-53); A-1 capability trim and editor/terminal write-time validation remain open there. LIMITATION: a CSP cannot restrict top-level navigation (CSP3's `navigate-to` was dropped from the spec with zero browser support), so `window.location` egress by any script running as `'self'` (most plausibly a compromised npm dependency, not XSS) stays open and needs a `WebviewWindowBuilder::on_navigation` allowlist instead; that guard is builder-time only, so it requires moving `main` window creation out of `app.windows` config into Rust `setup()`. Tracked as BL-NI-59 (Codex adversarial review). PENDING runtime verification against the running app (WebView renders, IPC works, dynamic swatch/bar styling survives, zero CSP-violation console errors) in both `tauri dev` and a `tauri build --debug`.
