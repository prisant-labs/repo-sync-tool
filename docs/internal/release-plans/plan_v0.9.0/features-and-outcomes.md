...
| Remove a repo | Stop tracking a repo (`repo_remove`). Clears the registry row and its history; it never touches the working tree on disk. | Curate the watch list without any risk to the actual clone. | Core done (backend + IPC only; **no GUI entry point yet**, see BL-NI-85) |
