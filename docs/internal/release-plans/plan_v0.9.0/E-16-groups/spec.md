...
- ~~The `repos_in_group(group_id)` batch query.~~ **Superseded 2026-07-05:** BL-NI-22 below is resolved, but as a bulk `repo_group_memberships()` read, not this literal query; see Known defects item 2.
...
- Membership data for the filter and the chips both come from `useRepoGroupMemberships` (`src/hooks/queries.ts`). **Updated 2026-07-05:** it now issues one bulk `repo_group_memberships` IPC call (not a per-repo fan-out) into the same `Map<repoId, groupId[]>` shape; see Known defect 2 / BL-NI-22 below.
...
## Known defects (all fixed 2026-07-05)

These six were real, verified bugs in the shipped vertical, surfaced by the 2026-07-04 Fable audit (`_LOCAL/audit/2026-07-04_18-21_fable-audit.md`). All six are now fixed - five in commit `4ab54bf`, the sixth (BL-NI-22) in commit `261a689`. Left in place (rather than deleted) so the as-built contract keeps an honest record of what shipped broken and how it was fixed; each entry below now carries its resolution.

1. **Group filter false-empties during membership load or on failure** (audit finding 7). `useRepoGroupMemberships` (`src/hooks/queries.ts`) exposes `data: Map | null`, `null` both while the fan-out is still loading and forever after it fails. In `src/screens/repos.tsx`'s `filtered` memo (line 110), `!membershipMap?.get(r.id)?.includes(activeGroupId)` treats a `null` map the same as "this repo is not a member," so with an active group filter the Repos list renders "No repositories match this filter" during every load and permanently if the fan-out ever fails, instead of a loading state or an error state. **FIXED (commit `4ab54bf`):** `inGroupCount` and the list body now special-case `membershipMap === null` and render the shared `AsyncPanel` loading/error presentation instead of the "no matches" empty state; the count banner shows "…" while membership is unknown.
2. **BL-NI-22 (O(N) group filter fan-out).** `useRepoGroupMemberships` issues one `groups_for_repo` IPC round-trip per visible repo rather than a single batched query. Fine at V1 repo counts, but it does not scale; the fix is a `repos_in_group(group_id) -> Vec<i64>` store function plus IPC command (or folding group ids directly into `RepoSummary`), additive to the schema and the IPC surface. Tracked in `docs/backlog.md` as BL-NI-22; do not duplicate the backlog entry, this spec only cross-references it. **RESOLVED (commit `261a689`; supersedes the `repos_in_group` suggestion above):** implemented as a single bulk `repo_group_memberships() -> Vec<RepoGroupMembership { repoId, groupIds }>` (core store fn + unit test in `store.rs`, additive IPC command, regenerated `bindings.ts`), not the literal `repos_in_group(group_id)`. One bulk call now powers BOTH the active-group filter and the per-row group chips in one IPC round-trip, since the old fan-out fed both; `repos_in_group` would only have served the filter. `useRepoGroupMemberships` (no longer taking a `repoIds` argument) and `src/screens/repos.tsx` were updated accordingly; the drawer's single-repo `groups_for_repo` call is unchanged (still the right tool for one open repo). See `docs/backlog.md` BL-NI-22 for the full resolution note.
3. **Group dialog double-Enter double-submits.** `GroupDialog`'s name `Input` calls `submit()` directly from its `onKeyDown` handler on Enter (`src/components/group-dialog.tsx` lines 111-113) with no guard against a second Enter firing before the first `await` resolves and `busy` re-renders as `true`. Two rapid Enter presses can issue two `group_create` (or `group_rename`) calls for the same input. **FIXED (commit `4ab54bf`):** `submit()` now returns immediately if `busy` is already `true`, guarding the re-entrancy window before the first `await` resolves.
4. **Duplicate-name error surfaces as "invalid setting: name" with wrong remediation.** `AppError::InvalidSetting`'s display message is `"invalid setting: {field}"` and its remediation is `"A setting has an invalid value. Correct it in Settings."` (`crates/reposync-core/src/error.rs`). For a duplicate group name this reaches the user verbatim via the dialog's error toast: a message written for the Settings screen, pointing at a Settings screen that has no group-name field at all, when the actual fix is to pick a different name in the group dialog that is still open. **FIXED at the UI layer (commit `4ab54bf`):** a new `isDuplicateNameError` check (keyed on the error code and the `field: "name"` context, not string-matching the message) makes the dialog toast "That name is already taken." instead of the raw `InvalidSetting` text. The underlying `AppError::InvalidSetting` message/remediation is unchanged (still Settings-flavored for any caller that surfaces it raw); the open question below on a generic or per-field remediation override for `InvalidSetting` itself is still E-05's to decide, but no longer user-visible for this path.
5. **Rename/delete buttons are focusable but invisible while keyboard-focused** (audit finding 12). In `GroupRow` (`src/components/groups-nav.tsx` line 188), the rename/delete icon buttons are revealed only via `opacity-0` to `group-hover/row:opacity-100`, a mouse-hover-only transition with no `focus-within`/`focus-visible` counterpart. A keyboard user who tabs to either button lands on a control that is present, focusable, and fully transparent. **FIXED (commit `4ab54bf`):** `group-focus-within/row:opacity-100` added alongside the existing hover reveal, plus a visible `focus-visible` ring on the row-icon buttons themselves.
6. **Delete-active-group force-navigates to Repos.** As described in Behavior on delete above, deleting the group that is the current active filter clears the filter through the same `onSelectGroup` path used for selecting a group, which unconditionally calls `setView("repos")` in `app-shell.tsx`. A user who opens the sidebar from Dashboard, Activity, or Settings and deletes their currently-active group filter is unexpectedly dropped onto the Repos screen. **FIXED (commit `4ab54bf`):** `GroupsNav` now takes a separate `onClearActiveGroup` prop (`app-shell.tsx`'s `clearActiveGroup`) that only clears `activeGroupId`, with no `setView` side effect; `doDelete` calls that instead of `onSelectGroup` when the deleted group was the active filter.
