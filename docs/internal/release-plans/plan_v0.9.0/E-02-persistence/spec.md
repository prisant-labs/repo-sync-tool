...
- **Recovery hardening (2026-07-11, DS-1 / BL-NI-50):** `init_pool_with_recovery` previously moved the database aside and started fresh on ANY open/migrate error (a deliberate uniform-treatment choice), so a transient lock or a buggy FUTURE migration would silently reset every user's DB. It now classifies the failure (`classify_init_failure`) and resets ONLY on a genuine corruption signature (`file is not a database` / `disk image is malformed`); a transient `SQLITE_BUSY`/`LOCKED` is retried (2x, 500ms backoff) then fails loud, and a version/checksum mismatch, a buggy migration, or an I/O error fails LOUD and leaves the DB untouched (the default branch is Fatal, so a reset requires an explicit corruption signal). Tests: `classify_init_failure_resets_only_on_corruption` (unit, all three buckets) + `init_pool_with_recovery_fails_loud_on_non_corruption` (a directory at the db path yields `Err`, no reset), with the existing corrupt-recovers + clean-start tests as regressions; clippy `-D warnings` clean. Follow-ups tracked in BL-NI-50: a CI migration test before any schema change, and turning the fatal-startup `.expect` (`src-tauri/src/lib.rs:267`) into a graceful error dialog.
