...
| WS | Delivers | Tier | Files / crates | Compiles shell crate? | Upstream deps |
|---|---|---|---|---|---|
| **P5-A** Version bump + doc sweep | `node scripts/bump-version.mjs 0.9.0` (all four version sources agree: root `Cargo.toml`, `src-tauri/Cargo.toml`, `package.json`, `src-tauri/tauri.conf.json`); finalize `CHANGELOG.md` `[Unreleased]` -> `## [0.9.0] - <date>`; bump README and install instructions; sweep `docs/architecture.md`/`explanation.md`/`faq.md` to remove "not built yet" hedges that no longer hold; mark shipped efforts in the roadmap; park V1.1 backlog items; write `docs/user-guide.md`, the enhanced explanatory user guide (jp directive 2026-07-04: install, first run, concepts - freshness model, update policies, cadence inherit, groups - every feature including tray, notifications, open-in, branch and PR intelligence, auto-update, plus troubleshooting; written against the SHIPPED behavior, explaining the why and not just the how). | Sonnet + Haiku | version files, `CHANGELOG.md`, `README.md`, `docs/**` | Rebuilds after bump | Phases 1-4 done (or descoped) |
