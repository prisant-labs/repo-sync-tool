...
### Commands - 21 of 22 real; only `summary_week` remains an intentional stub:
...
| `repo_set_cadence` | **DONE 2026-07-05 (P3-D)** | additive per-repo cadence write path (BL-NI-30 / audit finding 15): `repo_set_cadence(id, check_frequency_min)` where `0` = inherit the global cadence, a positive value = explicit per-repo override, a negative value is rejected (`InvalidSetting`). Persists via `store::repo_set_cadence`, then re-anchors `next_check_at` via the new `scheduler::reschedule_one_repo` (same anchored rule as the global-change reschedule, shared through `RESCHEDULE_SET_AND_GUARD`). Consumed by the repo-detail drawer's new "Check cadence" control |
| `db_recovery_notice` | **DONE 2026-07-05 (P3-D)** | additive read for the E-02 AC7 recovery notice (BL-NI-33): returns `DbRecoveryNotice { recovered, backup_path }` from the fields parked in `AppState`, so the frontend can surface a dismissible banner. Path rendered via a pure unit-tested `build_recovery_notice` helper |
...
| Piece | Owner | Task |
|-------|-------|------|
| Scheduler spawn | **DONE 2026-07-03 (commit 81c96af); gaps closed Phase 1 + P3-D** | the tokio scheduler spawns resident in `lib.rs` setup, with manual `check_now`/`update_now` routed through the same per-repo mutex as the scheduled path, so manual and scheduled work never collide on one repo. Audit finding 6 (no scheduler spawned when git is absent at startup) is resolved by BL-NI-23 (the loop spawns unconditionally and skips cleanly until a live git re-probe swaps an engine in). The daily activity retention sweep, previously startup-only, is now attached to the resident tick loop gated once per day (BL-NI-32, P3-D) |
