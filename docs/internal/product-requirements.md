...
**State-at-a-glance is the core requirement**, so the state vocabulary is stated precisely. Every state is encoded as color plus icon plus word, never hue alone, so it survives grayscale and color blindness. The states are: in sync, behind (`↓N`), ahead (`↑N`), dirty (skipped, reason stated), failed (with the specific error code and a remediation hint), paused (a first-class pill, not a greyed-out row), and new release. Delta counts are shown separately as `↑N` (ahead) and `↓N` (behind). The full taxonomy with encodings is in [features-and-outcomes.md](release-plans/plan_v0.9.0/features-and-outcomes.md) Section 8; the visual tokens are in [DESIGN.md](../../DESIGN.md).
