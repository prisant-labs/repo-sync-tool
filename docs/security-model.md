...
| Diagnostic logs are not redacted | The rotating logs under `logs/` record local filesystem paths and error text, and error text can include Git's own stderr. They inherit the same unredacted-retention question as the activity log above, and they are the more likely of the two to leave the machine, because a diagnostic log is the file a user attaches to a bug report. Bounded but not filtered: 14 daily files, 32 MiB total, oldest deleted first. Read one before sending it. | Open (BL-NI-54, same decision) |
