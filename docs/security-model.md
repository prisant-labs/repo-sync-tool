...
| Activity log is unbounded and unredacted | Raw Git stdout and stderr are persisted verbatim with no size cap. Attacker-controlled Git output can consume disk, and if authenticated GitHub access is added later, could retain a token. | Open (BL-NI-54) |
| Repository-local Git config partly trusted | `credential.helper` and `core.sshCommand` are not neutralized. | Open |
| Rust advisories are ungated | `cargo audit` currently reports two high-severity advisories in the locked graph and nothing in CI fails on them. | Gate pending |
