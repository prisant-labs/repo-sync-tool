...
| Logs and activity records still contain local paths | Captured Git output now passes through credential redaction at the point of capture, so it reaches the database, the UI, and the rotating logs already filtered. What redaction does NOT remove is local filesystem paths and repository names, which are inherent to the diagnostic value. A log or an exported activity record still describes your machine's layout. This matters most for the rotating logs under `logs/`, since a diagnostic log is the file people attach to bug reports. Read one before sending it. | Open, by design |
