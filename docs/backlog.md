...
| BL-NI-09 (CI build time vs release profile) | The Windows CI bundle took 35m15s, driven by `[profile.release]` (`opt-level="z"` + `lto=true` + `codegen-units=1`, tuned for the <30MB bundle). Per-push full-LTO Windows builds will slow every effort's CI. Candidate: thin LTO / higher codegen-units for per-push CI, reserving full LTO for release-tag builds via a dedicated cargo profile, accepting a small size increase on CI artifacts. Touches the <30MB V1 goal, so it is jp's call. | E-01 CI run (35m15s Windows leg). | open |
